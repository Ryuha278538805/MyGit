<%@ Page Title="" Language="C#" MasterPageFile="~/M/Index.Master" AutoEventWireup="true" CodeBehind="ls2.aspx.cs" Inherits="HCWDDL.M.ls2" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="BodyContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function search() {
            $.ajax({
                url: "/m/handler/NewsHandler.ashx", //处理页面的路径
                ...
                success: function (data) { //回调函数 ,成功时返回的数据存在形参data里
                    if (data && data.length > 0) {
                        for (var i = 0; i < data.length; i++) {
                            var row = data[i];
                            $("#ulArea").append('<li><a href="/m/newdetail.aspx?id=' + row.nid + '">'+row.title+'</a></li>');
                        }
                        pageIndex++;
                    }
                }
