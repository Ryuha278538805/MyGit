...
namespace Admin
{
    public partial class lottery_qxc : Pages
    {
        protected int id;
        protected int CzMode = (int)KjhCz.Czqxc;
        ...
        private void BinderList()
        {
            ...
            DataSet ds = KjhQxcData.GetPageListKjh(AspNetPager1.PageSize, AspNetPager1.CurrentPageIndex, "tbl_qxc", "qi desc", where);
        ...
        private void BinderKjhQxcInfo()
        {
            KjhQxcInfo info = KjhQxc.GetKjhQxcInfo(id, 0, false);
            if (info != null && info.id > 0)
            {
                ...
                zjt.Text = info.zjt.ToString().Trim();                
                jo1.Text = info.jo1.ToString().Trim();
                jo2.Text = info.jo2.ToString().Trim();
                jot.Text = info.jot.ToString().Trim();
        ...
        protected void enter_Click(object sender, EventArgs e)
        {
            ...
            int i = KjhQxc.CreateKjhQxcInfo(info);
            if (i > 0)
            {
                if (id == 0)
                    AlertMessage("提示：数据添加成功! ", "lottery_qxc.aspx");
                else
                    AlertMessage("提示：数据编辑成功! ", "lottery_qxc.aspx");
            }
            else if (i < 0)
        ...
        {
            int qi = Cz.GetNewQi((int)KjhCz.Czqxc);
            int getqi;
            int endqi = 155;
            //本年度到顶 过年了
            ...
            //写获取XML自动
            string url = "http://www.500.com/static/info/kaijiang/xml/qxc/" + getqi.ToString("00000") + ".xml";
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                info.zjt = ds.Tables[0].Rows[0]["Num1"].ToString().Replace(",", "");
                info.zj1 = ds.Tables[0].Rows[0]["Num2"].ToString().Replace(",", "");
                info.zj2 = ds.Tables[0].Rows[0]["Num3"].ToString().Replace(",", "");
                info.zj3 = ds.Tables[0].Rows[0]["Num4"].ToString().Replace(",", "");
                info.zj4 = ds.Tables[0].Rows[0]["Num5"].ToString().Replace(",", "");
                info.zj5 = ds.Tables[0].Rows[0]["Num6"].ToString().Replace(",", "");                
                info.jot = ds.Tables[0].Rows[0]["Money1"].ToString().Replace(",", "");
                info.jo1 = ds.Tables[0].Rows[0]["Money2"].ToString().Replace(",", "");
                info.jo2 = ds.Tables[0].Rows[0]["Money3"].ToString().Replace(",", "");                
                ...
                int i;
                i = KjhQxc.CreateKjhQxcInfo(info);
                if (i > 0)
                {
                    JsRedirect("lottery_qxc.aspx");
                    //if (id == 0)
        ...
        {
            int qi = Cz.GetMaxQi((int)KjhCz.Czqxc);
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                KjhQxcInfo info = KjhQxc.GetKjhQxcInfo(0, qi, false);
                info.qi = qi;
                info.a = TypeConverter.ObjectToInt(ds.Tables[0].Rows[0]["Result"].ToString().Split(',')[0]);
                info.b = TypeConverter.ObjectToInt(ds.Tables[0].Rows[0]["Result"].ToString().Split(',')[1]);
                info.c = TypeConverter.ObjectToInt(ds.Tables[0].Rows[0]["Result"].ToString().Split(',')[2]);
